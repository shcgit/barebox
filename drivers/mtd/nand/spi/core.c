...
{
	u8 status = 0;
	int ret = spinand_read_status(spinand, &status);
	if (ret)
		return ret;
...
static int spinand_mtd_block_markbad(struct mtd_info *mtd, loff_t offs)
{
	...
	return nanddev_markbad(nand, &pos);
}
...
static int spinand_mtd_block_isreserved(struct mtd_info *mtd, loff_t offs)
{
	...
	return nanddev_isreserved(nand, &pos);
}
...
static int spinand_probe(struct device *dev)
{
	struct spi_device *spi = (struct spi_device *)dev->type_data;
	struct spi_mem *mem = spi->mem;
	struct spinand_device *spinand;
	struct mtd_info *mtd;
	int ret;

	spinand = xzalloc(sizeof(*spinand));
	...
	ret = add_mtd_device(mtd, "nand", DEVICE_ID_DYNAMIC);
	if (!ret)
		return 0;
...
static struct driver spinand_drv = {
	.name = "spi-nand",
	.probe = spinand_probe,
	.of_compatible = DRV_OF_COMPAT(spinand_of_ids),
};
device_spi_driver(spinand_drv);
